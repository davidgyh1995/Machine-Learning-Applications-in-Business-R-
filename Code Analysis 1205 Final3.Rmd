...
colnames(Score) <- c("RMSE","De-norm RMSE")
...
Score["Linear Regression","De-norm RMSE"] = rmse(pred_test_lm,house_test$SalePrice)*(maxprice-minprice)+minprice
Score
```
```{r}
#0.191*(maxprice-minprice)+minprice
```
...
Score["Random Forest","De-norm RMSE"] = RMSE(predict_rf, house_test$SalePrice)*(maxprice-minprice)+minprice
Score
...
Score["Elastic Net","De-norm RMSE"] = RMSE(predictions, house_test$SalePrice)*(maxprice-minprice)+minprice
Score
```
...
Score["Boosting","De-norm RMSE"] = RMSE(pred.gbm.final, house_test$SalePrice)*(maxprice-minprice)+minprice
Score
```
...
test <- test[,!colnames(test) %in% c('Id','MSZoning','Street','Alley', 
                                     ...
                                     'SaleCondition', 'MasVnrArea')]
#Make prediction on unknown data set using Elastic Net 
# Predictor variables
...
final.pred <- predict(model, s=4411,newx=test,exact=TRUE)
final.pred
```
